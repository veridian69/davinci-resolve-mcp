...
Resolve session by tools/live_subtitles_validation.py instead -- the granular
package has no mock-based test convention to fit into, and inventing one here
would be its own maintenance burden for a project that already validates
this package live.
"""

import unittest

from src.granular.subtitles import build_subtitle_track_plan
...
class TrackPlanTests(unittest.TestCase):
    ...
    def test_single_speaker_does_not_need_explicit_track_routing(self):
        """Matches what tools/probe_subtitle_import.py measured: with only
        one subtitle clip, AppendToTimeline's simple list form is enough --
        it lands on the one subtitle track that exists. Explicit trackIndex
        is only needed to keep multiple speakers apart."""
