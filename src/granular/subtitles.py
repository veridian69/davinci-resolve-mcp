...
a re-transcribe of everything.
  ...
  whisperx_transcribe_timeline_item  wraps the backend in src/utils/media_analysis.py.
...
tests/test_subtitles_import.py. The Resolve calls around it -- AddTrack,
SetTrackName, ImportMedia, AppendToTimeline -- follow the measured shape from
tools/probe_subtitle_import.py: ImportMedia accepts .srt directly, and
AppendToTimeline's clipInfo form (already used by append_to_timeline in
media_pool.py) takes trackIndex explicitly. Validated end to end against a
live Resolve session by tools/live_subtitles_validation.py.
"""

import os

from src.granular.common import *  # noqa: F401,F403
from src.utils.media_analysis import _transcribe_with_whisperx
...
def build_subtitle_track_plan(written_files):
    ...
    exists, which is what tools/probe_subtitle_import.py measured. Explicit
    trackIndex only earns its complexity once there is more than one voice to
    keep apart.
    """
    plan = []
...
def whisperx_transcribe_timeline_item(
    ...
    Args:
        ...
        model, language, diarize, min_speakers, max_speakers, hf_token,
            ...
            command line -- see _whisperx_env in media_analysis.py.
    ...
    out_dir = output_dir or os.path.join(
        ...
        ".inproc", "subtitles", str(timeline_item_id))
...
def whisperx_import_subtitles(
    ...
    Measured against a live Resolve session (tools/probe_subtitle_import.py):
    ImportMedia accepts .srt directly and Resolve parses its cues correctly.
    A single speaker uses AppendToTimeline's simple list form; more than one
    uses the clipInfo form with explicit trackIndex so voices land on separate
    tracks instead of piling onto the same one.
