...
Separate from free_edition/boot/resolve_console_boot.py on purpose. The
dashboard (src/analysis_dashboard.py) calls
DaVinciResolveScript.scriptapp("Resolve") itself rather than reusing the
granular bridge's handle, so it needs the same shim installed but does not need
the MCP server running -- this boots the dashboard alone, on its own port, so
"Refresh Clips" and Overview can connect the way
free_edition/tools/probe_compound_connect.py measured that they do.

Run free_edition/boot/resolve_console_boot.py first if you also want the
343-tool MCP bridge; either order works, since this installs the shim itself
when it is not already there. Re-paste this same line to reload after editing
src/analysis_dashboard.py, any src/utils module it imports, or anything under
free_edition/.

INPROC_REPO names the CHECKOUT ROOT, not the directory this file sits in, and
the runtime state stays at <checkout>/.inproc/. Neither meaning changed when
this script moved under free_edition/boot/; only the paste line did.

The in-process card in the dashboard's MCP diagnostics panel is spliced into
upstream's HTML string at runtime by free_edition.integrate, so
src/analysis_dashboard.py stays byte-identical to upstream on disk.

ASCII-only, like free_edition/boot/resolve_console_boot.py: the console's
open() defaults to US-ASCII, and this file is read before any encoding fix can
exist.
"""
...
def _print_registrations(report):
    ...
    The card row is cosmetic and reports itself rather than raising: a missing
    diagnostics panel must not take the dashboard down. The rows above it are
    not cosmetic. src/analysis_dashboard.py captures `detect_capabilities` with
    a module-level from-import, so if it is not rebound here the dashboard's own
    analyses report whisperX absent, fall back to a backend that cannot diarize,
    and collapse every speaker into one subtitle track -- with nothing logged.
    """
    ...
    if card is not None:
        print(f"  dashboard card      : markup={card.get('markup')} "
              ...
              + (" (already installed)" if card.get("already_installed") else ""))
...
def _boot():
    if not REPO:
        raise BootFailed(
            ...
            'exec(open(INPROC_REPO+"/free_edition/boot/dashboard_console_boot.py").read())')
    ...
    # Purge before importing so edits to src/analysis_dashboard.py, any
    # src/utils module it imports, or anything under free_edition are picked up
    # on reload. Both packages: a purge that covered only src.* would reload
    # upstream while silently re-running the free-edition code the edit
    # replaced -- and here that includes the card markup itself, which would
    # look exactly like an edit that had no effect. Safe for free_edition
    # because this script is exec'd rather than imported, so it has no module
    # identity to destroy.
    ...
    # Idempotent: harmless to call even if free_edition/boot/resolve_console_boot.py
    # already installed the shim in this same console session.
    ...
    # The module object as well as the two names: install_dashboard_card()
    # rebinds dash.HTML and dash._mcp_status_payload, and a from-import alone
    # would leave nothing to rebind them on.
    ...
    # at runtime, and before the server thread starts. The payload wrapper is
    # re-read per request either way, but the HTML splice has to be in place for
    # the first page load.
    import free_edition.integrate as integrate

    print()
    registrations = integrate.install_all(console_resolve=_console_resolve,
                                          dash=dash, repo_root=REPO)
