...
# Registered at runtime by free_edition.integrate.register_subtitle_tools()
# rather than by an import line in src/granular/__init__.py, so nothing
# upstream has to know they exist. Nothing upstream notices if they do not
# get registered either, which is what this list is here to catch.
FREE_EDITION_TOOLS = ("whisperx_transcribe_timeline_item",
...
def run(console_resolve, granular_common, granular_mcp, handle, wrapped_tools,
        encoding_report, registrations=None):
    """Return a list of Check rows describing the live boot.

    `registrations` is the optional report dict from free_edition.integrate;
    when given it is echoed into the free-edition row. That row's verdict is
    read off the live tool registry either way, so a boot that passes nothing
    still gets the check.
    """
    from free_edition.inproc import api_probe, shim
    ...
    def free_edition_registrations():
        tools = getattr(getattr(granular_mcp, "_tool_manager", None),
                        "_tools", None) or {}
        missing = [n for n in FREE_EDITION_TOOLS if n not in tools]
        if missing:
            return False, ("not registered: " + ", ".join(missing)
                           + " -- integrate.register_subtitle_tools() "
                             "never ran, or ran against a different FastMCP")
        # dispatch.install() sets is_async on every tool it wraps and skips
        # tools that already carry it, so a False here means these two were
        # registered after it ran and now execute on the event-loop thread,
        # able to enter the single-threaded Resolve bridge concurrently.
        inline = [n for n in FREE_EDITION_TOOLS
                  if not getattr(tools[n], "is_async", False)]
        if inline:
            return False, (", ".join(inline) + " missed threaded dispatch "
                           "-- register_subtitle_tools() ran AFTER "
                           "dispatch.install(); re-run dispatch.install()")
        detail = (f"{len(FREE_EDITION_TOOLS)} subtitle tools registered "
                  "and wrapped")
        if registrations:
            detail += f"; integrate: {registrations}"
        return True, detail
    ...
    for name, fn in (
        ...
        ("free-edition registrations", free_edition_registrations),
