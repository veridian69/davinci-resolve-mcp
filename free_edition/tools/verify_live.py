...
no secret has to be copied around.
    ...
    /Library/Frameworks/Python.framework/Versions/3.14/bin/python3 \\
        free_edition/tools/verify_live.py [--no-write]
...
def _repo_root():
    ...
    This file lives three levels down (free_edition/tools/verify_live.py), so
    the walk is parents[2]. Getting that count wrong is completely silent: the
    transport handoff would be read from a path that never existed and this
    script would report "server not running" against a server that is.
    """
...
def load_endpoint():
    if not os.path.exists(STATE_PATH):
        raise SystemExit(
            ...
            f"  boot the server first: paste "
            f"free_edition/boot/resolve_console_boot.py in Resolve's console")
