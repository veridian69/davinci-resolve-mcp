...
Run AFTER the normal boot, so the shim and sys.path are already in place:
    ...
    exec(open("/path/to/davinci-resolve-mcp/free_edition/tools/probe_compound_inproc.py").read())
...
def main():
    ...
    if not shim_active:
        print()
        print("  Run free_edition/boot/resolve_console_boot.py first. Without")
        print("  the shim the import below will try the real bridge, get None,")
        print("  and tell you nothing about whether the compound server itself")
        print("  is viable.")
        return
    ...
    try:
        ...
        print("    python3 free_edition/tools/setup_inproc.py "
