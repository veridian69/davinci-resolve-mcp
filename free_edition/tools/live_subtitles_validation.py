...
Unlike the other free_edition/tools/probe_*.py scripts, this one needs a real
timeline item with real audio -- there is no way to synthesize speech from
inside Resolve's embedded interpreter the way tools that ran outside it could
shell out to macOS `say`. So this picks the first audio-bearing item off the
...
are already in place:
    ...
    exec(open("/path/to/davinci-resolve-mcp/free_edition/tools/live_subtitles_validation.py").read())
...
WHAT THIS DOES TO YOUR PROJECT
  ...
  see free_edition/README.md) and, unlike the scratch-and-clean
  free_edition/tools/probe_subtitle_import.py, LEAVES the resulting subtitle
  tracks in place rather than deleting them -- the point is to look at them.
  Uses --model tiny for speed; diarization is off by default since it needs an
  HF_TOKEN this script does not manage (set DIARIZE = True and export HF_TOKEN
  yourself to exercise that path).
...
def main():
    ...
    if getattr(bridge, "__file__", None) != "<inproc-shim>":
        print("shim not installed -- run "
              "free_edition/boot/resolve_console_boot.py first")
        return

    granular = __import__("sys").modules.get("src.granular")
    if granular is None or not hasattr(granular, "mcp"):
        print("src.granular not imported -- run "
              "free_edition/boot/resolve_console_boot.py first")
        return

    from free_edition.subtitles.tools import (
    ...
    if item_id:
        from free_edition.subtitles.tools import _find_timeline_item
