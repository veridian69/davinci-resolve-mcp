...
sound; it says nothing about whether a tool does the right thing to a timeline.
    ...
    /Library/Frameworks/Python.framework/Versions/3.14/bin/python3 \\
        free_edition/tools/fake_console.py
"""
...
def _repo_root():
    ...
    This file lives three levels down (free_edition/tools/fake_console.py), so
    the walk is parents[2]. Getting that count wrong is completely silent: the
    boot below is handed REPO as INPROC_REPO, so a bogus root would be inherited
    by the simulated console and surface much later, one layer away, as a
...
REPO = _repo_root()
BOOT = os.path.join(REPO, "free_edition", "boot", "resolve_console_boot.py")
...
# The probe name free_edition/inproc/selftest.py asks about. The real bridge
# returns None for every absent name; this fake cannot enumerate Resolve's whole
# surface, so it declares absence for a named set instead and keeps answering
# everything else. Same observable behaviour where it counts.
...
class PyRemoteObject:
    ...
    free_edition/inproc/api_probe.py keys off type(obj).__name__, so the name
    matters. Reproduces what free_edition/tools/probe_api_identity.py measured
    on Resolve 21.0.3.7: an absent method comes back as None WITHOUT raising,
    which is precisely why builtins.hasattr reports it as present.
    """
