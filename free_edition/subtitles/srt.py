...
apart from `timing`, which stays purely numeric.
"""
...
def write_speaker_srt_files(payload, out_dir, prefix=""):
    ...
    """
    # The one line in this module that reaches into upstream. segments_to_srt
    # is pristine upstream code and stays there, so this module is not
    # stdlib-only: the checkout root must be on sys.path by the time this
    # function is called. Deferred rather than module-level for the same
    # reason it always was -- importing media_analysis is expensive and the
    # boot purges and re-imports `src.*` in a deliberate order.
